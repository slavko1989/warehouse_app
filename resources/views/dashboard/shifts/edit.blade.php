            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="post" action="{{ route('edit_shift',['id' => $edit->id]) }}">
                                @csrf

                                <div class="mb-3">
                                    <label for="date" class="form-label">Date</label>
                                    <input type="date" class="form-control"  name="day_in_week"
                                    value="{{ $edit->day_in_week }}">
                                    @error('day_in_week')
                                ...
                                <div class="mb-3">
                                    ...
                                    @foreach($emp as $e)
                                    <option value="{{ $e->id }}" {{ $e->id == session('selected_employee_id') ? 'selected' : '' }}>{{ $e->full_name }}</option>
                                    @endforeach
                                    </select>
                                </div>
